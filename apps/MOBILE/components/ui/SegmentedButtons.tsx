...
export function SegmentedButtons({
  ...
  return (
    <View
      ...
      className={`border-border flex-row overflow-hidden rounded-full border ${className}`}
    >
      {buttons.map((button, index) => {
        ...
        return (
          <Pressable
            ...
            style={styles.button}
            className={`flex-1 flex-row items-center justify-center p-3 ${selected ? 'bg-primary/10' : 'bg-transparent'} ${button.disabled ? 'opacity-50' : ''} ${!isLast ? 'border-border border-r' : ''} `}
          >
            {button.icon && (
              <View className={button.label ? 'mr-2' : ''}>{button.icon}</View>
            )}
            {button.label && (
              <Text
                style={{
