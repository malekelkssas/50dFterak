  ...
  return (
    <Pressable
      ...
      className={`items-center justify-center rounded-md ${
        isChecked ? 'bg-primary/15' : 'bg-transparent'
...
function ToggleButtonRow({
  ...
  return (
    <ToggleGroupContext.Provider value={{ value, onValueChange }}>
      <View
        className={`border-secondary/30 flex-row items-center overflow-hidden rounded-lg border ${className}`}
      >
        {React.Children.map(children, (child, index) => (
          <View
            className={`${index > 0 ? 'border-secondary/30 border-l' : ''}`}
          >
            {child}
          </View>
        ))}
