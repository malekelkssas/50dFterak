...
function FloatingLabel({
  ...
  return (
    <Animated.Text
      ...
      className={`${
        error ? 'text-red-500' : isFocused ? 'text-primary' : 'text-secondary'
...
function TextInputRoot({
  ...
  const modeWrapperClass =
    mode === 'outlined'
      ? `border rounded-md px-3 pt-4 pb-2 ${
          error
            ? 'border-red-500'
            : isFocused
  ...
  return (
    <View className={`${className}`} style={style}>
      <View
        className={`relative flex-row items-center ${modeWrapperClass} ${
          disabled ? 'opacity-50' : ''
        } bg-muted`}
      >
      ...
      {error && errorText && (
        <Text className="mt-1 ml-3 text-xs text-red-500">{errorText}</Text>
      )}
    </View>
  );
}
...
function TextInputIcon({ icon, onPress, style }: TextInputIconProps) {
  ...
  return (
    <View className="p-1" style={style}>
      {icon}
    </View>
  );
}
...
function TextInputAffix({ text, textStyle }: TextInputAffixProps) {
  return (
    <Text style={textStyle} className="text-secondary text-base">
      {text}
    </Text>
  );
}
